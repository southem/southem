...
function removeAnimationsFromStyle(style) {
  return omitBy(style, (value, key) => isFunction(value) && endsWith(key, ANIMATION_SUFFIX));
}
...
function transferAnimatedValues(styleValue, animatedStyleValue) {
  if (isFunction(animatedStyleValue.interpolate) || isUndefined(styleValue)) {
...
function resolveAnimatedStyle({
  ...
  if (!isFunction(createAnimatedStyle)) {
  ...
  return mergeWith(removeAnimationsFromStyle(style), animatedStyle, transferAnimatedValues);
}
...
export function connectAnimation(WrappedComponent, animations = {}, options = defaultOptions) {
  ...
  class AnimatedComponent extends PureComponent {
    ...
    onLayout(event) {
      ...
      if (!isEqual(layout, this.state.layout)) {
