...
export const INCLUDE = '@@southem.theme/include';
...
function includeSymbolMergeHandler(objVal, srcVal) {
  ...
  if (isUndefined(newObjVal) && isPlainObject(srcVal)) {
    ...
    const newObj = mergeWith({}, srcVal, (o, s) => s);
  ...
  if (isPlainObject(newObjVal) && include) {
...
export default function resolveIncludes(target, base = {}) {
  ...
  function includeNodeStyles(styleNode, processingStyleNames) {
    ...
    if (styleNamesToInclude) {
      if (!isArray(styleNamesToInclude)) {
    ...
    const resultingStyle = mergeWith({}, stylesToInclude, styleNode, includeSymbolMergeHandler);
    delete resultingStyle[INCLUDE];

    for (const styleName of keys(resultingStyle)) {
