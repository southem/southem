...
function createDocsJson(): NodeJS.Process {
  return gulp.src(['packages/components/**/*.tsx', '!src/components/**/*.spec.tsx'])
    .pipe(typedoc({
...
function createDocsInputJson(done: GulpCompletionCallback): void {
  ...
  const evaThemes: DocThemes = createDocThemesForPackage('themes', 'light', 'dark');
...
function rebuildDocApp(done: GulpCompletionCallback): void {
  exec('yarn run clean && yarn run build', { cwd: PLAYGROUND_DIR });
