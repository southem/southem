...
describe('resolveIncludes', () => {
  describe('resolve only target ', () => {
    it('includes styles', () => {
      ...
      expect(includedStyle.includeSingleStyle.number)
        .toEqual(target.numberInclude.number);

      expect(includedStyle.includeMultipleStyles.number)
        .toEqual(target.numberInclude.number);

      expect(includedStyle.includeMultipleStyles.text)
        .toEqual(target.textInclude.text);
    });
    ...
    it('includes nested includes', () => {
      ...
      expect(includedStyle.includingStyle.number)
        .toEqual(target.secondIncludeNumber.number);

      expect(includedStyle)
        .toMatchObject(expectedResolvedStyle);
    });
    ...
    it('target style overrides it self properly', () => {
      ...
      expect(includedStyle.includeAndOverride.number)
        .toEqual(target.includeAndOverride.number);

      expect(includedStyle.includeAndOverride.text)
        .toEqual(target.textInclude.text);
    });
  });
  ...
  describe('resolve target with base', () => {
    describe('resolve base depending includes', () => {
      it('includes styles from base', () => {
        ...
        expect(includedStyle.includeSingleStyleFromBase.number)
          .toEqual(base.numberInclude.number);

        expect(includedStyle.includeMultipleStyles.number)
          .toEqual(base.numberInclude.number);

        expect(includedStyle.includeMultipleStyles.text)
          .toEqual(base.textInclude.text);
      });
      ...
      it('throws error if base style has include and is required', () => {
        ...
        expect(() => { resolveIncludes(target, base); })
          .toThrow();
      });
    });
    ...
    describe('resolve target and base depending includes', () => {
      it('includes styles from both', () => {
        ...
        expect(includedStyle.includeMultipleStyles.number1)
          .toEqual(base.numberInclude.number1);

        expect(includedStyle.includeMultipleStyles.text1)
          .toEqual(base.textInclude.text1);
      });
      ...
      it('both overrides target style properly', () => {
        ...
        expect(includedStyle.includeAndOverride.text1)
          .toEqual(target.includeAndOverride.text1);

        expect(includedStyle.includeAndOverride.text)
          .toEqual(target.textInclude.text);

        expect(includedStyle.includeAndOverride.number1)
          .toEqual(base.numberInclude.number1);
      });
    });
  });
  ...
  describe('resolves invalid include values', () => {
    it('successfully with non existing include value', () => {
      ...
      expect(includedStyles.testInclude.test)
        .toEqual(target.testInclude.test);
    });
    ...
    it('without error if include is "not"', () => {
      ...
      expect(resolveIncludes(undefinedInclude)).toMatchObject(undefinedInclude);
      expect(resolveIncludes(falseInclude)).toMatchObject(falseInclude);
    });
  });
});
